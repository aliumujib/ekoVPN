...
class ModuleDownloaderPreference(context: Context, attrs: AttributeSet?) : Preference(context, attrs) {
    ...
    override fun onClick() {
        ...
        WireGuardInitializer.getAsyncWorker().supplyAsync(WireGuardInitializer.getModuleLoader()::download).whenComplete(this::onDownloadResult)
    }
    ...
    private fun onDownloadResult(result: Int, throwable: Throwable?) {
        when {
            ...
            result == OsConstants.EXIT_SUCCESS -> {
                ...
                WireGuardInitializer.getSharedPreferences().edit().remove("disable_kernel_module").commit()
