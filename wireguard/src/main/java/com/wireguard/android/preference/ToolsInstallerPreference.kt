...
class ToolsInstallerPreference(context: Context, attrs: AttributeSet?) : Preference(context, attrs) {
    ...
    override fun onAttached() {
        super.onAttached()
        WireGuardInitializer.getAsyncWorker().supplyAsync(WireGuardInitializer.getToolsInstaller()::areInstalled).whenComplete(this::onCheckResult)
    }
    ...
    override fun onClick() {
        ...
        WireGuardInitializer.getAsyncWorker().supplyAsync { WireGuardInitializer.getToolsInstaller().install() }.whenComplete { result: Int, throwable: Throwable? -> onInstallResult(result, throwable) }
    }
